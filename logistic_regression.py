...
def calculate_logistic_loss(model, X, y):
    """Calculates logistic loss for the given data."""
    probabilities = model.predict_proba(X)
    probabilities = np.clip(probabilities, 1e-10, 1 - 1e-10)  # Avoid log(0)
...
def do_experiments(start, end, step_num):
    ...
    plt.subplot(3, 3, 2)
    plt.plot(shift_distances, beta1_list, marker='o', label='Beta1 (Coefficient for x1)', color='green')
    ...
    plt.subplot(3, 3, 3)
    plt.plot(shift_distances, beta2_list, marker='o', label='Beta2 (Coefficient for x2)', color='orange')
    ...
    plt.subplot(3, 3, 5)
    plt.plot(shift_distances, [-x for x in intercept_list], marker='o', label='Intercept (Beta0 / Beta2)', color='red')
    ...
    # Plot logistic loss
    plt.subplot(3, 3, 6)
    plt.plot(shift_distances, loss_list, marker='o', label='Logistic Loss', color='brown')
    ...
    plt.subplot(3, 3, 7)
    plt.plot(shift_distances, margin_widths, marker='o', label='Margin Width', color='cyan')
